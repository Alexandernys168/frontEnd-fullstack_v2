...
public class UserController {
    ...
    public void RegisterUser(@RequestBody UserVm userVm){
        switch (userVm.getUserType()){
            case "DOCTOR": doctorService.create(new Doctor(userVm.getId(), userVm.getName(), userVm.getEmail(), userVm.getPassword()));break;
    ...
    public UserVm loggInUser(@RequestBody UserVm userVm){
        List<UserVm> userVms = getAllUsers();
        for (UserVm u:userVms) {
            if (Objects.equals(u.getEmail(), userVm.getEmail()) && Objects.equals(u.getPassword(), userVm.getPassword())){
                return u;
            }
        }
